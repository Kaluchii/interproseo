<div class="box">
    <div class="box-header with-border">
        <h3 class="box-title">Входит в стоимость {{$item->sorter_field}}</h3>
        <button type="submit" class="btn btn-primary pull-right remove-flat-item" data-id="{{$item->id_field}}"
                data-block="works">Удалить</button>
    </div>
    <div class="box-body">
                        ...
                        <div class="input-group file-caption-main">
                            ...
                            <div class="file-preview-frame">
                                <div class="kv-file-content">
                                    <img src="{{$item->work_field->link}}?{{$item->work_field->cache_index}}"
                                         class="kv-preview-data file-preview-image"
                                         title="{{$item->work_field->alt}}"
                                         alt="{{$item->work_field->alt}}">
                                </div>
                                <div class="file-thumbnail-footer">
                                    <div class="file-footer-caption"
                                         title="{{$item->work_field->alt}}">{{$item->work_field->name_field}}
                                        <br><samp>(425.24 KB)</samp></div>
                                    <input type="text" class="form-control alt-text" data-block="works"
        ...
        <div class="form-group">
            ...
            <input class="form-control string"
                   ...
                   value="{{$item->work_title_field}}"
                   data-name="work_title"
                   data-type="string"
                   data-block="works"
                   data-id="{{$item->id_field}}">
        </div>
